#!/bin/bash
sudo apt-get install seclists feroxbuster crowbar crackmapexec rubeus neovim chisel bloodhound freerdp3-proxy netexec -y
sudo sed -i 's/\/cert-ignore/\/cert:ignore/g' /usr/share/crowbar/lib/main.py
